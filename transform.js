...
function transform({ siteContent, city: cityOverride } = {}) {
  ...
  const fields = {
    ...
    top_bar_items: ((src.top_bar || {}).items || []).map((x) => ({ text: x })),
    header_logo: (src.top_bar || {}).logo || null,
    // These sites reuse the header logo in the footer — there's no separate
    // footer logo widget in the source, so without this the footer always
    // falls back to the plain-text logo.
    footer_logo: (src.top_bar || {}).logo || null,
    ...
    hero_bottom_subtitle: hero.subtitle || null,
    ...
    hero_bg_image: firstNonBgImage(hero.images),
  ...
  sections.forEach((s, i) => {
    switch (dests[i]) {
      ...
      case 'about': {
        ...
        fields.about_image = firstNonBgImage(s.images);
      ...
      case 'why_choose':
        ...
        fields.about_side_image = firstNonBgImage(s.images);
        break;
      ...
      case 'costs': {
        ...
        fields.costs_side_image = firstNonBgImage(s.images);
        const items = (s.lists || []).flatMap((l) => l.items);
      ...
      case 'services_intro': {
        ...
        // the 3 keyword columns → services_usps (theme caps at 4)
        const all = (s.lists || []).flatMap((l) => l.items);
        if (all.length) fields.services_usps = all.slice(0, 4).map((t) => ({ text: t }));
        break;
      }
  ...
  // Why-Choose-Us has no image of its own on some sites (no matching section,
  // or a why-choose section with no image widget) — reuse the hero image
  // rather than leaving the theme's camera placeholder showing.
  if (!fields.about_side_image) fields.about_side_image = fields.hero_bg_image || null;
  ...
  // stale values from earlier pushes never survive.
  const MANAGED = [
    'global_phone_display','global_phone_clean',
    'top_bar_items','footer_description','header_logo','footer_logo','footer_copyright',
    'header_cta_text','header_cta_link',
    'hero_title','hero_bottom_title','hero_bottom_subtitle','hero_usp_items',
    ...
    'services_heading','services_subheading','services_usps','services_list',
